/* =========================================================
   ...
   Rent visningslag: den vet ikke hva som er riktig svar før
   den får beskjed om det (fra QuestionManager, via main.js).
   Spilleren får bare ett svarforsøk per spørsmål.
   ========================================================= */
...
export class QuestionView {
  constructor(panelEl) {
    ...
    this.bonusEl = panelEl.querySelector('[data-role="bonus"]');
  ...
  askChoice(question) {
    ...
    this.bonusEl.textContent = `+${question.bonusMoves} TREKK`;
  ...
  async showFeedback({ buttons, chosenIndex, correct, correctIndex, bonusMoves, word }) {
    ...
    if (correct) {
      ...
      this.feedbackTextEl.textContent = `+${bonusMoves} trekk`;
    } else {
